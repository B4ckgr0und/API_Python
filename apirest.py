...
@app.route('/api/usuario/<string:emp>', methods = ['GET'])
def usuario(emp):
    try:
        nombre_store_procedure = 'sp_lista_empleados_02'
        sql_query = text(f'exec {nombre_store_procedure} @emp=:emp')
        resultados = db.session.execute(sql_query,{'emp': emp})
        usuarios_json = [dict(row._asdict()) for row in resultados]
        return jsonify({'usuarios':usuarios_json})
    except Exception as e:
        return jsonify({'Error':str(e)})
